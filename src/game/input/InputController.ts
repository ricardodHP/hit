...
export class InputController {
  ...
  private touch = { x: 0, y: 0, attack: false, skill: false, skill2: false, skill3: false, dodge: false, block: false };
  private touchRoot?: HTMLElement;
  constructor(private scene: Phaser.Scene) {
    const keyboard = scene.input.keyboard!;
    this.cursors = keyboard.createCursorKeys();
    this.keys = keyboard.addKeys('W,A,S,D,J,K,O,P,L,I,R,F2') as Record<string, Phaser.Input.Keyboard.Key>;
  ...
  getState(): InputState {
    ...
    return { moveX: Phaser.Math.Clamp(moveX, -1, 1), moveY: Phaser.Math.Clamp(moveY, -1, 1), attack: Phaser.Input.Keyboard.JustDown(this.keys.J) || Phaser.Input.Keyboard.JustDown(this.cursors.space) || this.touch.attack || Boolean(pad?.A), skill: Phaser.Input.Keyboard.JustDown(this.keys.K) || this.touch.skill || Boolean(pad?.B), skill2: Phaser.Input.Keyboard.JustDown(this.keys.O) || this.touch.skill2 || Boolean(pad?.X), skill3: Phaser.Input.Keyboard.JustDown(this.keys.P) || this.touch.skill3 || Boolean(pad?.Y), dodge: Phaser.Input.Keyboard.JustDown(this.keys.L) || this.touch.dodge || Boolean(pad?.R1), block: this.keys.I.isDown || this.touch.block || Boolean(pad?.L1) || Boolean(pad?.R2), debug: Phaser.Input.Keyboard.JustDown(this.keys.F2), restart: Phaser.Input.Keyboard.JustDown(this.keys.R) };
  }
  clearMomentaryTouch(): void { this.touch.attack = false; this.touch.skill = false; this.touch.skill2 = false; this.touch.skill3 = false; this.touch.dodge = false; }
  private createTouchControls(): void {
    const root = document.getElementById('hud-root'); if (!root) return;
    const wrap = document.createElement('div'); wrap.className = 'touch-controls'; wrap.innerHTML = `<div class="touch-pad"><span></span><button class="touch-button" data-dir="up">▲</button><span></span><button class="touch-button" data-dir="left">◀</button><span></span><button class="touch-button" data-dir="right">▶</button><span></span><button class="touch-button" data-dir="down">▼</button><span></span></div><div class="touch-actions"><button class="touch-button" data-act="attack">J</button><button class="touch-button" data-act="skill">K</button><button class="touch-button" data-act="skill2">O</button><button class="touch-button" data-act="skill3">P</button><button class="touch-button" data-act="dodge">L</button><button class="touch-button" data-act="block">I</button></div>`;
    ...
    wrap.querySelectorAll<HTMLButtonElement>('button').forEach(b=>{b.onpointerdown=()=>{const d=b.dataset.dir; const a=b.dataset.act; if(d) setDir(d,true); if(a==='attack') this.touch.attack=true; if(a==='skill') this.touch.skill=true; if(a==='skill2') this.touch.skill2=true; if(a==='skill3') this.touch.skill3=true; if(a==='dodge') this.touch.dodge=true; if(a==='block') this.touch.block=true;}; b.onpointerup=b.onpointercancel=()=>{const d=b.dataset.dir; if(d) setDir(d,false); if(b.dataset.act==='block') this.touch.block=false;};});
  }
