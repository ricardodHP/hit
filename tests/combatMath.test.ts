...
import { enemyConfigFor } from '../src/game/actors/Enemy';
...
describe('pure combat math', () => {
  ...
  it('keeps normal and heavy enemy config distinct', () => { const normal=enemyConfigFor('normal'); const heavy=enemyConfigFor('heavy'); expect(heavy.maxHealth).toBeGreaterThan(normal.maxHealth); expect(heavy.speed).toBeLessThan(normal.speed); });
